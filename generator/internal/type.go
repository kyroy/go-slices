...
// Map creates a new slice with the results of calling the provided function on every element in the calling array.
func (s {{ .Package }}) Map(f func(s {{ .Type }}) {{ .Type }}) {{ .Package }} {
	return Map(s, f)
}

// Filter creates a new slice with all elements that pass the test implemented by the provided function.
func Filter(s []{{ .Type }}, f func(s {{ .Type }}) bool) {{ .Package }} {
	m := {{ .Package }}(make([]{{ .Type }}, 0, len(s)))
	for _, v := range s {
		if f(v) {
			m = append(m, v)
		}
...
// Filter creates a new slice with all elements that pass the test implemented by the provided function.
func (s {{ .Package }}) Filter(f func(s {{ .Type }}) bool) {{ .Package }} {
	return Filter(s, f)
}
...
func (s {{ .Package }}) Reduce(f func(sum, value {{ .Type }}) {{ .Type }}, neutral {{ .Type }}) {{ .Type }} {
	return Reduce(s, f, neutral)
}

// Unique creates a new slice without duplicate values.
func Unique(s []{{ .Type }}) {{ .Package }} {
	m := {{ .Package }}(make([]{{ .Type }}, 0, len(s)))
...
// Intersect creates a new slice of values that are included in all given arrays.
// The order of values in the result is random.
func (s {{ .Package }}) Intersect(more ...[]{{ .Type }}) {{ .Package }} {
